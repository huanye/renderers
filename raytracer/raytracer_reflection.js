window.onload = function(){
	...
	var sphere1 = new Sphere([0,-1,3],1,[255,0,0],500,0.2);
	var sphere2 = new Sphere([2,0,4],1,[0,0,255], 500,0.3);
	var sphere3 = new Sphere([-2,0,4],1,[0,255,0],10,0.4);
	//add a big yellow sphere
	var sphere4 = new Sphere([0,-5001,0],5000,[255,255,0],1000,0.5);
	...
	var ReflectRay= function(R,N){
		return Subtract(Smul(2*Dot(N,R), N),R);
	};
	...
	// function to compute the closest intersection
	var ClosestIntersection = function(O,D,t_min,t_max){
	...
	var ComputeLighting= function(P,N,V,s){
		...
		for(var i=0; i<lights.length;i++){
			...
			if(lights[i].type == "ambient"){
				...
				var shadow_sphere;
				var shadow_t;
				[shadow_sphere, shadow_t] = ClosestIntersection(P, light_direction, 0.001,t_max);
	...
	var TraceRay = function(O,D,t_min,t_max,recursion_depth){
		
		var intersection = ClosestIntersection(O,D,t_min,t_max);
		...
		// compute local color
		var local_color =  Smul(ComputeLighting(P,N,V,closest_sphere.specular),closest_sphere.color);
		...
		//compute the reflective color
		var R = ReflectRay(Smul(-1,D),N);
		var reflected_color = TraceRay(P,R,0.001,window.Infinity,recursion_depth-1);
		return Add(Smul(1-r,local_color),Smul(r,reflected_color));
	};
	...
	for (var x=-Math.floor(canvas.width/2);x<=Math.floor(canvas.width/2);x++){
		for (var y=-Math.floor(canvas.height/2);y<=Math.floor(canvas.height/2);y++){
			...
			color = TraceRay(O,D,1,window.Infinity,3);
			PutPixel(x,y,color);
		}
	}
	UpdateCanvas();
